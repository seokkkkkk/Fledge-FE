...
const CanaryModal = ({ onClick }: CanaryModalProps) => {
  ...
  // useMutation을 사용하여 데이터 제출 및 applyStatus를 갱신
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (applyData.userId) {
        ...
        return postCanaryApply(uploadApplyData, accesstoken);
      }
    },
    onSuccess: () => {
      // postCanaryApply가 성공적으로 완료되었을 때 applyStatus 다시 가져오기
          ...
          <div className="input-section main-text">
            <div className="sub-section">
              <div className="w-[662.79px]">
                <span className="sub-text">보호종료확인서 업로드</span>
                <ImageInput>
            ...
            <div className="sub-section">
              <div className="w-[303.19px]">
                ...
                <Input
                  ...
                  onChange={(e) => handleApplyData(e.target.value, "phone")}
              ...
              <div className="birth">
                <DropDown
                  ...
                  onChange={(e) => handleBirthData(e.target.value, "year")}
                />
                <DropDown
                  type="month"
                  width="110px"
                  value={birthData.month + "월"}
                  onChange={(e) => handleBirthData(e.target.value, "month")}
                />
                <DropDown
                  type="day"
                  width="110px"
                  value={birthData.day + "일"}
                  onChange={(e) => handleBirthData(e.target.value, "day")}
                />
              </div>
              <div>
                <DropDown
                  hint="성별"
                  items={["남성", "여성"]}
                  value={applyData.gender === true ? "남성" : "여성"}
                  onChange={(e) =>
